...
export const articuloSeeder = async () => {
    ...
    const getMarcaId = (nombre: string) =>
        marcas.find((m) => m.nombre.toLowerCase() === nombre.toLowerCase())?.idMarca;

    const getGeneroId = (nombre: string) =>
        generos.find((g) => g.nombre.toLowerCase() === nombre.toLowerCase())?.idGenero || null;

    const getSubSubCategoria = (nombre: string) =>
        subSubcategorias.find(s => s.nombre.toLowerCase() === nombre.toLowerCase())?.idSubSubcategoria

    const asignarGenero = (tipo: "ropa" | "calzado" | "otro") => {
